...
from analyze_results import add_compare_scores_columns, print_info_performance_overview, \
    extract_tuned_hyperparameter_from_models, add_performance_improvement_column_and_plot
...
def _generate_features():
    for X_file_name in [X_CLEAN_FILE_NAME, X_FILTERED_FILE_NAME]:
        print(f"Generate Features based on the data of: {X_file_name}")
        ...
        # set train, test filenames according to the Type of X (clean, filtered, etc.)
        if X_file_name == X_CLEAN_FILE_NAME:
            train_filename = X_TRAIN_CLEAN_AUTOENCODER_FILE_NAME
            test_filename = X_TEST_CLEAN_AUTOENCODER_FILE_NAME

        elif X_file_name == X_FILTERED_FILE_NAME:
            train_filename = X_TRAIN_CLEAN_FILTERED_AUTOENCODER_FILE_NAME
            test_filename = X_TEST_CLEAN_FILTERED_AUTOENCODER_FILE_NAME
        ...
        create_features(
            ...
            test_filename=test_filename,
            datasets_folder=DATASETS_FOLDER_PATH,
            transformer_params=AUTOENCODER_PARAMS,
            prefix="autoencoder_",
            random_state=RANDOM_STATE,
            X_file_name=X_file_name,
            y_file_name=Y_FILE_NAME,
...
if __name__ == "__main__":
    ...
    add_performance_improvement_column_and_plot(results_file_path=RESULTS_FILE_PATH)
